...
- **CAPS Lesson Plan Generator**: Teacher dashboard "Lesson Plan" tab — teacher selects Subject, Grade, Term, Week, Duration, and optional Topic. Calls `generateCAPSLessonPlan()` in `gemini.ts` which returns structured JSON. Displays CAPS alignment, prior knowledge, objectives, resources, colour-coded lesson phases (teacher & learner activities), assessment, differentiation (support/extension), homework, and teacher reflection prompt. Printable PDF with school branding, EMIS, and CAPS-ALIGNED badge.
- **Noticeboard & Gallery**: Admin Dashboard → Communication → "Noticeboard & Gallery". Admin creates notices (📢) and gallery items (🖼️) with title, content, image URL, pin-to-top, and show/hide toggle. All visible items are fetched dynamically on the public Home page: pinned notices appear first; gallery items appear in a responsive grid below the notices section. Stored in `timetable_config.notices` JSONB array.
- **SMS History**: After every successful SMS send, a record (date, audience, grade, recipient count, message) is saved to `timetable_config.sms_history` (last 100 kept). The history table appears at the bottom of the SMS tab in Admin Dashboard.
- **Online Application Form**: Public `/admissions` page now has a real form (student info, DOB, grade, parent info, phone, email, extra notes). On submit the application is saved to `timetable_config.applications` with status `pending`. Admin Dashboard → Admissions → Applications shows all submissions with Accept / Reject buttons; clicking either updates the status and sends an automatic SMS to the parent's phone number (if SMS is configured).
